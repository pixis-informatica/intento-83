...
(function () {
  ...
  const getProductMeta = card => {
    ...
    const description = card?.dataset.desc ? card.dataset.desc.replace(/&#10;|\n/g, ' ').trim() : defaultDescription;
    const image = normalizeImage(card?.dataset.img);
    const url = window.location.origin + window.location.pathname + '?producto=' + getSlug(title);
    return { title, description, image, url };
  };

  const applyMeta = ({ title, description, image, url }) => {
    createMeta({ id: 'pixis-og-title', attr: 'property', name: 'og:title', content: title });
    createMeta({ id: 'pixis-og-description', attr: 'property', name: 'og:description', content: description });
    createMeta({ id: 'pixis-og-image', attr: 'property', name: 'og:image', content: image });
    createMeta({ id: 'pixis-og-url', attr: 'property', name: 'og:url', content: url });
    createMeta({ id: 'pixis-twitter-card', attr: 'name', name: 'twitter:card', content: 'summary_large_image' });
...
(function () {
  ...
  // To remove: comment out this IIFE or remove the code
})();
